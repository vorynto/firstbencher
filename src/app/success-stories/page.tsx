                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {stories.map((story) => (
                            <div key={story.id} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all flex flex-col gap-5">
                                ...
                                {/* 1. Profile image + name/course/company */}
                                <div className="flex items-center gap-4">
                                    {story.image_url ? (
                                        <img src={story.image_url} alt={story.student_name} className="w-16 h-16 rounded-full border-2 border-white shadow-md object-cover flex-shrink-0" />
                                    ) : (
                                ...
                                <div className="flex gap-1 text-yellow-400">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} size={18} className={i < (story.rating || 5) ? "fill-current" : "text-gray-200"} />
                                    ))}
                                </div>
                                ...
                                {story.certificate_url && (
                                    <div className="pt-1">
                                        ...
                                        <a href={story.certificate_url} target="_blank" rel="noopener noreferrer" className="block rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                                            <img
